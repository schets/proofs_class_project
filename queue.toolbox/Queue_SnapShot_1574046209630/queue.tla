...
begin
 Write:
  while write_ind <= Writes do
    ...
    buffer[write_ind % BufSize] := write_ind;
 ...
 Read:
  while read_ind <= Writes do
    if read_ind < write_ind then
        last_read := buffer[read_ind % BufSize];
        read_ind := read_ind + 1;
    end if;
  end while;
...
Write == /\ pc[0] = "Write"
         /\ IF write_ind <= Writes
               THEN /\ buffer' = [buffer EXCEPT ![write_ind % BufSize] = write_ind]
                    /\ write_ind' = write_ind + 1
...
Read == /\ pc[1] = "Read"
        /\ IF read_ind <= Writes
              THEN /\ IF read_ind < write_ind
                         THEN /\ last_read' = buffer[read_ind % BufSize]
                              /\ read_ind' = read_ind + 1
                         ELSE /\ TRUE
                              /\ UNCHANGED << read_ind, last_read >>
                   /\ pc' = [pc EXCEPT ![1] = "Read"]
...
\* Last modified Sun Nov 17 22:03:19 EST 2019 by samuelschetterer
